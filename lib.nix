...
rec {
  ...
  mkSnabbBenchTest = { name, times, ... }@attrs:
   ...
   listDrvToAttrs = l: builtins.listToAttrs (map (attrs: lib.nameValuePair (lib.replaceChars ["."] [""] attrs.name) attrs) l);

   # Snabb fixtures
   mkNixTestEnv = { kernel ? linuxPackages, dpdk ? linuxPackages.dpdk }:
   let
