...
in rec {
  ...
  */
  mkMatrixBenchNFVIperf = { snabb, times, qemu, kPackages, conf ? "NA", hardware ? "lugano",  ... }:
    ...
    in mkSnabbBenchTest {
      name = "iperf_conf=${conf}_snabb=${software.versionToAttribute snabb.version or ""}_kernel=${software.versionToAttribute kPackages.kernel.version}_qemu=${software.versionToAttribute qemu.version}";
      inherit hardware kPackages snabb times qemu;
      testNixEnv = software.mkNixTestEnv { inherit kPackages; };
      toCSV = drv: ''
  ...
  */
  mkMatrixBenchNFVDPDK = { snabb, qemu, kPackages, dpdk, hardware ? "lugano", times, pktsize ? "", conf ? "", ... }:
    ...
    then []
      ...
      name = "l2fwd_pktsize=${pktsize}_conf=${conf}_snabb=${software.versionToAttribute snabb.version or ""}_dpdk=${software.versionToAttribute dpdk.version}_qemu=${software.versionToAttribute qemu.version}";
      inherit snabb qemu times hardware dpdk kPackages;
      needsNixTestEnv = true;
      testNixEnv = software.mkNixTestEnv { inherit kPackages dpdk; };
      toCSV = drv: ''
