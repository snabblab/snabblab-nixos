...
This report makes a basic comparison of how several Snabb versions perform. The benchmarks used are "end to end" functional tests. Only one set of software versions are considered for the dependencies used in test cases (QEMU, Linux kernel, DPDK).
...
* `l2fwd` measures the throughput in packets per second when looping packets from a physical network through a Linux virtual machine running the DPDK `l2fwd` example program. Each packet is received from the network, forwarded to the VM, received from the VM, and sent back onto the network. The score shows the number of packets that complete the loop.
